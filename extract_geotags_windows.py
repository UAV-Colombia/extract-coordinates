...
def main():
    ...
    layout = [
        [sg.Text("Enter the folder path:")],
        [sg.Input(), sg.FolderBrowse()],
        [sg.Button("Extract Coordinates")],
    ]
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED:
            break
        elif event == "Extract Coordinates":
            folder_path = values[0]
            coordinates = extract_coordinates_from_images(folder_path)
            ...
            sg.popup(f"Coordinates saved to {output_file}.")
            break
